...
bool D3DClass::Initialize(int screenHeight, int screenWidth, bool vsync, HWND hwnd, bool fullscreen, float deephscreen, float nearscreen)
{
	...
	if (FAILED(result))
	{
		MessageBox(hwnd, L"Could not create a DirectX 11.0 device.  The default video card does not support DirectX 11.0.", L"DirectX Device Failure", MB_OK);
		return false;
	}
